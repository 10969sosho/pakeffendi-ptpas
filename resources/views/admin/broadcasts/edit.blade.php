...
@section('content')
    <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm w-full">
        <form method="post" action="{{ route('admin.broadcasts.update', $broadcast) }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="4" required class="w-full rounded-lg border border-slate-200 focus:border-sky-500 focus:ring-sky-500">{{ old('description', $broadcast->description) }}</textarea>
            </div>
